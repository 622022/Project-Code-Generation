...
public class AccountsApiController implements IAccountsApi {
    ...
    @PreAuthorize("hasAuthority('EMPLOYEE')")
    public ResponseEntity<AccountObject> editAccount(@ApiParam(value = "The account to be updated of specific user", required = true) @Valid @RequestBody AccountObject body
    ...
    public ResponseEntity<AccountObject> getSpecificAccount(@ApiParam(value = "the iban of the requested account.", required = true) @PathVariable("IBAN") String IBAN
        ...
        try {
            return new ResponseEntity<AccountObject>(accountService.getSpecificAccount(IBAN), HttpStatus.OK);
        } catch (Exception e) {
            LOGGER.warning("Could not get account" + e.getMessage());
            return new ResponseEntity<AccountObject>(HttpStatus.NOT_FOUND);
        }
    }
}